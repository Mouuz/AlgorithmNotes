...
- Floyd 算法的正确性可以从动态规划的角度进行证明。我们假设 $dis[i][j]$ 表示节点 $i$ 到节点 $j$ 的最短路径长度，$k$ 表示路径上可能经过的中间节点，则 Floyd 算法可以表示为：

  $$
  dis[i][j] = \min\{dis[i][j], dis[i][k] + dis[k][j]\}
  $$

  其中 $\min$ 表示取两个值的最小值。这个公式的意义是，如果经过中间节点 $k$ 能够使得从 $i$ 到 $j$ 的路径长度更小，那么就更新 $dis[i][j]$ 的值。

  我们可以证明，当 $k=0,1,2,..., n-1$ 时，$dis[i][j]$ 的值已经表示了从 $i$ 到 $j$ 经过 $k$ 个中间节点（不重复）的所有可能路径中最短的一条。

  证明过程如下：

  - 当 $k=0$ 时，$dis[i][j]$ 表示只经过源节点的路径长度，即 $dis[i][j]=w(i,j)$。
  - 当 $k=1$ 时，$dis[i][j]$ 表示经过源节点和一个可能的中间节点 $1$ 的路径长度，即 $dis[i][j]=\min\{w(i,j), w(i,1)+w(1,j)\}$。由于存在可能的中间节点 $1$，因此上面的公式可以正确地求出 $i$ 到 $j$ 的最短路径长度。
  - 继续类似地推导下去，当 $k=2$ 时，$dis[i][j]$ 再加上一个可能的中间节点 $2$，以此类推，最终可以求出从 $i$ 到 $j$ 经过不重复中间节点的所有可能路径中最短的一条。

  综上所述，Floyd 算法是正确的，因为它考虑了所有可能的路径，并找出了最短的一条。

- 在下面代码中，判断从a到b是否是无穷大距离时，需要进行**if(t > INF/2)**判断，而并非是**if(t == INF)**判断，原因是INF是一个确定的值，并非真正的无穷大，会随着其他数值而受到影响，**t**大于某个与INF相同数量级的数即可,和上面的Bellman_ford算法结束的判断同理。
...
//如果有重边则在存图时通过  dist[i][j] = min(dist[i][j], w) 来只保留两点间最短的边即可		
